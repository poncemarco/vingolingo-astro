...
export default function Toast({ name, quantity, itsKg }: { name: string, quantity: number, itsKg: boolean}) {
    ...
    useEffect(() => {
        ...
        return () => {
          // Clear the timeout when the component is unmounted or when it disappears
    ...
    return isVisible ? (
        ...
        <div className="ms-3 text-sm font-normal">Se agregaron {quantity} {itsKg ? "KG" : null} {name}</div>
        <button type="button" onClick={() => setIsVisible(!isVisible)} className="ms-auto -mx-1.5 -my-1.5 bg-white text-gray-400 hover:text-gray-900 rounded-lg focus:ring-2 focus:ring-gray-300 p-1.5 hover:bg-gray-100 inline-flex items-center justify-center h-8 w-8 dark:text-gray-500 dark:hover:text-white dark:bg-gray-800 dark:hover:bg-gray-700" data-dismiss-target="#toast-default" aria-label="Close">
